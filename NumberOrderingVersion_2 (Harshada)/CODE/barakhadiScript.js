...
NumberOrdering.setAllImages=function()
{
	...
	$('#FinalOk').prop('src',Utils.Path+'IMAGES/doll.gif');
	$('#Next').prop('src',Utils.Path+'IMAGES/Barakhadi_Next.png');
	$('#level1').prop('src',Utils.Path+'IMAGES/green-Leaf-with-1.png');
	$('#level2').prop('src',Utils.Path+'IMAGES/green-Leaf-with-2.png');
...
NumberOrdering.init=function()
{
	...
	$('#background').css({'background-image':"url('IMAGES/Cover.jpg')"});
...
*/
	...
	$('.align6InLine').css({/*'border':'3px solid black',*/'color':'transparent','background-image':'url('+Utils.Path+'IMAGES/yellow-Leaf.png)','background-repeat': 'no-repeat','background-size': 'contain'});
...
NumberOrdering.checkData=function()
{
	for(var i=0;i<NumberOrdering.arrOfNumbers.length;i++)
	{
		if($('#space'+i).data('Answer')==$('#div'+i).data('Answer'))
		{
			...
			$('#space'+i).parent().css({'background-image':'url('+Utils.Path+'IMAGES/green-Leaf.png)'});
		}
	}
};
...
{
	var destiDiv,sourceLetter,destiLetter,sourceDiv,sourceLetterData,destiLetterData, next, prev, current
	...
	if(Number($(sourceDiv).prop('id').substring(3))>Number($(destiDiv).prop('id').substring(3)))
	{
		var sourceIndex=parseInt($(sourceDiv).prop('id').substring(3));
		var destiIndex=parseInt($(destiDiv).prop('id').substring(3));
		current=$('#div'+sourceIndex+' > p').text();
		for(var i=destiIndex;i<sourceIndex;i++)
		{
			//destiIndex++;
			next=$('#div'+(i+1)+'> p').text();
			$('#div'+(i+1)+'> p').text(current);
			$('#div'+(i+1)+'> p').data('Answer',current);
			$('#div'+(i+1)+'> p').draggable();
			current = next;
		}
	}
	...
	if(sourceDiv!==destiDiv)
	{
		...
		for(var i=0;i<NumberOrdering.arrOfNumbers.length;i++)
		{
			if($('#div'+i+'> p').data('Answer')== $('#div'+i).data('Answer'))
			{
				j++;
				$('#div'+i).css({'background-image':'url('+Utils.Path+'IMAGES/green-Leaf.png)'});
				//$('#div'+i+'> p').css({'color':'green'});
			}
			else{
				$('#div'+i).css({'background-image':'url('+Utils.Path+'IMAGES/yellow-Leaf.png)'});
				//$('#div'+i+'> p').css({'color':'black'});
			}
		}
		NumberOrdering.answerCounter=j;
	}
