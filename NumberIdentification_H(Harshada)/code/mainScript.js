...
NumberIdenti.setAllImages=function()
{
	...
	$('#level1').prop('src',Utils.Path+'Images/Level-1.png');
	$('#level2').prop('src',Utils.Path+'Images/Level-2.png');
	...
	$('#girl').prop('src',Utils.Path+'Images/Girl.png');
	$('#boy').prop('src',Utils.Path+'Images/Boy.png');
};
...
NumberIdenti.fillNumberChart=function(element)
{
	...
	if(element==level1)
	{
		...
		for(var i=0;i<number;i++)
		{
			...
			$(div).css({'background-image':'url('+Utils.Path+'Images/'+(i+1)+'.png)','background-repeat': 'no-repeat','background-size': '100% 100%'});
