...
Barakhadi.addSpacesForBarakhadi=function()
{
	...
	for(var i=0;i<2;i++)
	{
		...
		for(var j=0;j<6;j++)
		{
			...
			{
				spanElement=$('<span id="space'+k+'" class="trans spaces" style="font-size:60px;color:black;padding:12px;">'+Barakhadi.ltrForbarakhadi['list'][k]['word']+'</span>');
...
Barakhadi.Handledrop=function(event,ui)
{
	...
	if($(draggedLetter).text().trim()==$(droppedSpan).text().trim())
	{
		...
		document.getElementById('playSound').src='SOUND/BuzzerWrong.mp3';
		document.getElementById('playSound').play();
	}
}
