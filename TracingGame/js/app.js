...
document.getElementById("part3").addEventListener("click",startlevel3);
    ...
    function startlevel2() {
		 if(!tracing.deviceFlag){
			...
			Android.audioPlayer("TracingGame/"+imgSoundMappingMain[1].soundURL);
		}
		 document.getElementById("heading").style.display='block';
         $("#level2").show();
         $("#mainpage").hide();
    ...
    function startlevel3() {
		 if(!tracing.deviceFlag){
			...
			Android.audioPlayer("TracingGame/"+imgSoundMappingMain[2].soundURL);
		 }
		 document.getElementById("heading").style.display='block';
         $("#level3").show();
         $("#mainpage").hide();
...
function setTextForLevel2(){
         ...
         cntx.strokeText(imgTextMapping[position].originalText, can2.width/2, can2.height/2+60);
		 currentI=position;
...
soundButton1.addEventListener("click", function (event) {
	if(!tracing.deviceFlag){
			...
			Android.audioPlayer("TracingGame/"+imgShapeMapping[tracing.cnt].soundURL);
		}
});	
...
soundButton2.addEventListener("click", function (event) {
	if(!tracing.deviceFlag){
			...
			Android.audioPlayer("TracingGame/"+imgTextMapping[currentI].soundURL);
		}
});	
...
soundButton3.addEventListener("click", function (event) {
	if(!tracing.deviceFlag){
			...
			Android.audioPlayer("TracingGame/"+imgNumberMapping[currentI].soundURL);
		}
});	
...
function level3(){
 ...
 function genericTextLevel2(i){
			 ...
			 cntx.font = "bold 200px Arial";
			 cntx.textAlign = "center";
			 cntx.strokeText(imgTextMapping[i].originalText, can2.width/2, can2.height/2+60);
 }
 ...
 function setMarker(r,g,b,canvasName){
        $(canvasName).Sketch.color="#0F0F0F";
 }