(function () {
    ...
    var tracing = [];
		...
		$(".background1").css("background-image","url('"+Utils.Path +"Content/Cover.png')");
        $(".background2").css("background-image","url('"+Utils.Path +"Content/Inside-page.png')");
        $("#R1").attr("src", Utils.Path + "Content/joker_01.png");
        $("#W2").attr("src", Utils.Path + "Content/joker_02.png");
        $("#W3").attr("src", Utils.Path + "Content/joker_03.png");
    }
    ...
    cmbapp.controller('CombController', function ($scope, $interval) {
        ...
        $scope.onDrop = function (target, source) {
            ...
            if (source == "ं" || source == "ः" || source == "़")
                $scope.words[target] = $scope.words[target].concat(source);
            else if($scope.words[target][1]=="़" && source != "़")
                $scope.words[target] = $scope.words[target].concat(source);
            else
		...
		this.changeword = function () {
			...
			console.log(this.mainword.WordBank);
		    this.RightWord1();
        }
