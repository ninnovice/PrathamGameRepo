...
{
	$('#Flip,#wrongimg1,#wrongimg2').hide();
	$('#coverPage').show();
	$('#playGame').text(EnglishFlip.hardCodedData.playGame);
	$('#FlipGameForRI').css({'backgroundImage':"url('Images/ulatPalat.gif')"});
};

EnglishFlip.getGameData=function()
{
	var randomIndex=0,tempArray=[];
	EnglishFlip.counter=0,EnglishFlip.correctAnswerCount=0,EnglishFlip.arrOfSelectedWords=[];
...
EnglishFlip.showQuestions=function()
{
...
EnglishFlip.startGame=function()
{
	$('#Flip').show();
	$('#coverPage').hide();
	$('#FlipGameForRI').css({'backgroundImage':"url('Images/NG.jpg')"});
	//EnglishFlip.getGameData();
...
EnglishFlip.ifAllAnswCorrect=function()
{
